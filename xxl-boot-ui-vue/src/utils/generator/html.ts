...
let confGlobal: (FormConf & { fields: FormItemConf[] }) | null
...
function attrBuilder(el: FormItemConf) {
  return {
...
// el-buttin 子级
function buildElButtonChild(conf: FormItemConf): string {
...
// el-input innerHTML
function buildElInputChild(conf: FormItemConf): string {
  const children: string[] = []
  if (conf.prepend) {
...
function buildElRadioGroupChild(conf: FormItemConf): string {
  const children: string[] = []
  if (conf.options && conf.options.length) {
...
function buildElCheckboxGroupChild(conf: FormItemConf): string {
  const children: string[] = []
  if (conf.options && conf.options.length) {
...
function buildElUploadChild(conf: FormItemConf): string {
  const list: string[] = []
