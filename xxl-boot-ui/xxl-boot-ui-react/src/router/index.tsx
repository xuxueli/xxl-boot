  ...
  // 主布局：首页默认跳转工作台；业务路由由编排（buildAppRoutes）注入 children
  {
    ...
    children: [
      // 首页默认跳转工作台（路径读取 default-settings 配置，与 Vue redirect: defaultSettings.homePath 对齐）
      {
        index: true,
        element: (
          <Navigate to={defaultSettings.homePath ?? '/dashboard'} replace />
        ),
      },
    ],
  },
...
for (const [modulePath, factory] of Object.entries({
  ...
  pageMap.set(
    relative,
    factory as () => Promise<{ default: React.ComponentType }>,
  );
}
...
export const buildAppRoutes = (
  ...
  const [loginRoute, errorRoute, notFoundRoute, homeRoute, userRoute] =
    constantRoutes;
  return [
    // 登录、301
    loginRoute,
    ...
    {
      ...homeRoute,
      children: [...(homeRoute.children ?? []), ...dynamicRoutes],
    } as RouteObject,
    // 个人中心
    userRoute,
