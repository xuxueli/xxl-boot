...
service.interceptors.request.use((config) => {
    ...
    requestConfig.metadata = {skipErrorHandler: headers.skipErrorHandler === true};
    ...
    // 2. 防重复提交（POST/PUT）
    if (!isRepeatSubmit && (config.method === 'post' || config.method === 'put') && config.data) {
        // FormData / Blob 无法序列化比对，跳过
        if (config.data instanceof FormData || config.data instanceof Blob) return config;
        ...
        if (JSON.stringify(snapshot).length >= REPEAT_SUBMIT_MAX_SIZE) return config;
...
service.interceptors.response.use(
    ...
    (response): any => {
        ...
        if (code !== 200) {
            const msg =
                res?.msg || errorCode[String(code)] || errorCode.default;
