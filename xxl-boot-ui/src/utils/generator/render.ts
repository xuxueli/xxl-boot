...
const componentChild: Record<string, Record<string, ChildRenderFn>> = {
  ...
  'el-select': {
    options(h, conf, key) {
      return conf.options.map((item: any) => h(resolveComponent('el-option'), {
        label: item.label,
        value: item.value,
      }))
    }
  ...
  'el-radio-group': {
    options(h, conf, key) {
      return conf.optionType === 'button' ? conf.options.map((item: any) => h(resolveComponent('el-checkbox-button'), {
        label: item.value,
      }, () => item.label)) : conf.options.map((item: any) => h(resolveComponent('el-radio'), {
        label: item.value,
        border: conf.border,
      }, () => item.label))
    }
  ...
  'el-checkbox-group': {
    options(h, conf, key) {
      return conf.optionType === 'button' ? conf.options.map((item: any) => h(resolveComponent('el-checkbox-button'), {
        label: item.value,
      }, () => item.label)) : conf.options.map((item: any) => h(resolveComponent('el-checkbox'), {
        label: item.value,
        border: conf.border,
      }, () => item.label))
    }
...
export default defineComponent({
  ...
  render() {
    ...
    return h(resolveComponent(self.conf.tag),
      {
