...
- 针对逻辑相关的一组ref，比如 `form`、`rules`、`loading`，统一放在一个对象中，避免散落在各处。如 `views/system/log/index.vue` 中代码。
- 针对读写自动与父组件 v-model:xxx 同步的场景，使用 `defineModel`。如 `views/system/log/index.vue、detail.vue` 中代码。
