...
- 4、【优化】升级IP地理位置解析工具，重构工具类使用新版API进行初始化和查询；
- 5、【优化】Maven依赖管理优化，统一各模块依赖版本号引用，便于集中维护管理
- 6、【优化】代码生成SQL解析逻辑优化，解决字段类型大小写识别不兼容问题；
- 7、【ING】新增插件模块“Agent应用”：
  - Agent配置管理，支持多Agent类型，包括：Chat对话、图片生成、视频生成、语音合成...等；支持多模型供应商，包括：Ollama、OpenAI、Aliyun、Doubao...等。
