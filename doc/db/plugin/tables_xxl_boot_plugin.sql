...
-- ================== ai：model、chat、message ==================

CREATE TABLE `xxl_boot_ai_model`
(
    `id`                INT             NOT NULL AUTO_INCREMENT      COMMENT 'Model ID',
    `name`              VARCHAR(100)    NOT NULL                     COMMENT 'Model名称',
    `model_type`        TINYINT         NOT NULL                     COMMENT 'Model类型：1-基础模型，1-文本模型，2-视觉模型',
    `supplier_type`     TINYINT         NOT NULL                     COMMENT '供应商类型：1-Ollama',
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT ='Model 配置';

CREATE TABLE `xxl_boot_ai_chat`
(
    `id`            INT             NOT NULL AUTO_INCREMENT      COMMENT 'Chat ID',
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT ='Chat 对话表';
...
(
    ...
    `sender_type`       TINYINT         NOT NULL                     COMMENT '发送者类型：1-Model、2-用户',
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT ='Chat Message 对话消息明细表';
...
VALUES (1, 'Ollama模型（qwen）', 1, 1, 'qwen3.5:2b', 'http://127.0.0.1:11434', '', now(), now()),
       ...
       (3, 'OpenRouter模型（free）', 1, 2, 'openrouter/free', 'https://openrouter.ai/api/v1/chat/completions', 'xxx', now(), now());
...
VALUES (200, 0, 'AI应用', 0, 'ai', '/ai', 'fa-fire', 200, 0, 0,now(), now()),
       (201, 200, '对话管理', 1, 'ai:chat', '/ai/chat', '', 201, 0, 0,now(), now()),
       (202, 200, '知识库管理', 1, 'ai:kb', '/ai/kb', '', 202, 0, 0,now(), now()),
       (203, 200, '模型配置', 1, 'ai:model', '/ai/model', '', 203, 0, 0,now(), now());
